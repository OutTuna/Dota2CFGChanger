...
void ui_render_main(ImFont* font_big, ImVec2 ds) {
    ...
    if (open_src) {
        ...
        if (!p.empty()) { strncpy(src_path, p.c_str(), PATH_BUF_SIZE - 1); src_path[PATH_BUF_SIZE - 1] = 0; }
    }
    if (open_dst) {
        open_dst = false;
        auto p = browse_for_folder("Куда конфиг");
        if (!p.empty()) { strncpy(dst_path, p.c_str(), PATH_BUF_SIZE - 1); dst_path[PATH_BUF_SIZE - 1] = 0; }
    }
